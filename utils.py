...
def get_hpatches_data(files, norm, **config):
    ...
    def _adapt_homography_to_preprocessing(zip_data):
        ...
        source_size = tf.cast(zip_data['shape'], dtype=tf.float32)
        source_warped_size = tf.cast(zip_data['warped_shape'], dtype=tf.float32)
        target_size = tf.cast(tf.convert_to_tensor(config['preprocessing']['resize']), dtype=tf.float32)

        # Compute the scaling ratio due to te resizing for both images
        ...
        pad_y = tf.cast((source_size[0]*s - target_size[0]) / tf.constant(2.), dtype=tf.int32)
        pad_x = tf.cast((source_size[1]*s - target_size[1]) / tf.constant(2.), dtype=tf.int32)
        ...
        pad_y = tf.cast((source_warped_size[0]*warped_s - target_size[0]) / tf.constant(2.), dtype=tf.int32)
        pad_x = tf.cast((source_warped_size[1]*warped_s - target_size[1]) / tf.constant(2.), dtype=tf.int32)
