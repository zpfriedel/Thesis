...
from homographies import sample_homography, flat2mat, invert_homography
...
with open('configs/config_patches_generation.yaml', 'r') as f:
    config = yaml.load(f)

basepath = Path('/home/ubuntu/data', 'COCO/val/')
image_paths = list(basepath.iterdir())
output_dir = Path('/home/ubuntu/data', 'COCO/patches/')
...
for num, path in enumerate(image_paths):
    ...
    # Run
    im, warped_im, homography = sess.run([image, warped_image, H], feed_dict={tf_path: str(path)})
