...
def homography_adaptation(image, model, config):
    """
    Perfoms homography adaptation. Inference using multiple random warped patches of the same input image for robust
    predictions

    Arguments:
        image: tensor with shape [N, H, W, 1]
    ...
    probs = probs[:, :, :, :-1]
    probs = tf.depth_to_space(probs, block_size=8)
    ...
    return {'prob': prob, 'counts': counts, 'input_images': images, 'H_probs': probs}  # debug
...
def sample_homography(
    ...
    Return:
        homography: a tensor of shape [1, 8] corresponding to the flattened homography transform
    """
    ...
    if scaling:
        ...
        if allow_artifacts:
            ...
            valid = tf.cond(tf.shape(valid)[0] < 1, lambda: tf.cast(tf.range(n_scales), tf.int64), lambda: valid)
    ...
    if rotation:
        ...
        if allow_artifacts:
            ...
            valid = tf.where(tf.reduce_all((rotated >= 0.) & (rotated < 1.), axis=[1, 2]))[:, 0]
            valid = tf.cond(tf.shape(valid)[0] < 1, lambda: tf.cast(tf.range(n_angles), tf.int64), lambda: valid)
...
def compute_valid_mask(image_shape, homography, erosion_radius=0):
    ...
    Arguments:
        ...
        homography: Tensor of shape (N, 8) or (8,), where N is the batch size
        erosion_radius: radius of the margin to be discarded
    Return:
        mask: a tensor of type `tf.int32` and shape (H, W)
    """
...
def warp_points(points, homography):
    ...
    Arguments:
        points: list of points, shape [num_keypoints, 2].
        homography: batched or not (shapes [N, 8] and [8,] respectively)
    Return:
        warped_points: a tensor of shape [num_keypoints, 2] or [N, num_keypoints, 2]
...
def warp_keypoints_to_list(packed_arg):
    """
    Warp a map of keypoints with the INVERSE of the homography H.
    The inverse is used to be coherent with tf.contrib.image.transform

    Arguments:
        packed_arg: a tuple equal to [keypoint_map, H]
    Return:
        warped_keypoints: a tensor of size [num_keypoints, 2] with the new coordinates of the warped keypoints
    """
...
def warp_keypoints_to_map(packed_arg):
    ...
    Arguments:
        packed_arg: a tuple equal to [keypoint_map, H]
    Return:
        new_map: a map of keypoints of the same size as the original keypoint_map
    """
