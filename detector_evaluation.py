...
def draw_keypoints(img, corners, color, radius=3, s=3):
    img = np.repeat(cv.resize(img, None, fx=s, fy=s)[..., np.newaxis], 3, -1) * 255.
    for c in np.stack(corners).T:
        cv.circle(img, tuple(s*np.flip(c, 0)), radius, color, thickness=-1)
    return img
...
for i in range(1):
    for e, thresh in zip(experiments, confidence_thresholds):
        path = Path('/home/ubuntu/data', 'exports', e, str(i) + '.npz')
        d = np.load(path)
...
for thresh in thresholds:
    ...
    for exp in experiments:
        repeatability = compute_repeatability(exp, keep_k_points=300, distance_thresh=thresh, verbose=True)
